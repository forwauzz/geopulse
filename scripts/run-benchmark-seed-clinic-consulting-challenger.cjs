...
const configuredEnvRoot = process.env.GEO_PULSE_ENV_ROOT
  ? path.resolve(process.env.GEO_PULSE_ENV_ROOT)
  : root;
const workspaceParent = path.resolve(root, '..', '..');
const envRoot = existsSync(path.join(configuredEnvRoot, '.env.local'))
  ? configuredEnvRoot
  : workspaceParent;
const result = spawnSync(process.execPath, [
  ...
  tsx,
  path.resolve(__dirname, 'benchmark-seed-clinic-consulting-challenger.ts'),
