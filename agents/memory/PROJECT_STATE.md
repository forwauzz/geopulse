...
| BM-041 | Define competitor/cohort benchmark methodology, guardrails, and evidence limits before any broader comparative benchmark claims or cohort UI are implemented | Architect + Product | ⬜ PENDING | — |
| BM-042 | Add the first competitor/cohort benchmark storage and admin slice for a narrow internal cohort only, after BM-041 freezes methodology | Backend + Frontend | ⬜ PENDING | — |
| BM-043 | Add the first benchmark query/response/citation lineage view so one run can be inspected as a real benchmark graph rather than disconnected tables | Backend + Frontend | ⬜ PENDING | — |
| BM-044 | Add the first multi-model benchmark lane beyond the current single configured live lane while keeping provider abstraction and admin controls simple | Backend | ⬜ PENDING | — |
| BM-045 | Add benchmark-scale operator hardening for 100–200 domain internal runs with explicit isolation, retry policy, and failure visibility before any 500–1000 site push | Backend + Ops | ⬜ PENDING | — |
| BM-046 | Revisit the 500–1000 site benchmark operations path only after BM-045 evidence exists and operator cost/failure tradeoffs are documented | Architect + Ops | ⬜ PENDING | — |
