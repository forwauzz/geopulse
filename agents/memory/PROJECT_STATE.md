...
Current truth: the benchmark admin flow now supports an opt-in live Gemini lane for one configured model via `BENCHMARK_EXECUTION_*` env, while default behavior remains the safe stub adapter. Completed responses flow through citation parsing and metric computation, admins can inspect recent runs, drill into a run group, view one domain’s benchmark history over time, add a benchmark domain from the admin UI, add a lightweight query set from the admin UI, and launch one benchmark run from the benchmark overview UI. Live provider failures now preserve the Gemini error body in `response_metadata`, and a run group is marked `failed` when every query run fails. First live benchmark proof now exists for a real customer domain with 6 completed queries, 4 extracted citations, and non-zero metrics on `gemini-2.5-flash-lite`; temporary `503 UNAVAILABLE` responses remain an expected provider-side retry case, not a benchmark-foundation blocker.
...
| BM-024 | Add light retry/backoff handling for temporary benchmark-provider overload (`503 UNAVAILABLE`) before marking a query run failed | Backend | ⬜ PENDING | — |
