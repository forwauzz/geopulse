...
function EmailPreview({ activity }: { readonly activity: GrowthCalendarActivity }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-outline-variant/25 bg-surface-container-lowest shadow-float">
      {/*
        The From line comes from the activity, never from a constant. A hardcoded address here
        would show the founder an authenticated GEO-Pulse identity that does not exist yet.
      */}
      {[['From', activity.senderLine ?? 'GEO-Pulse outreach sender'], ['To', activity.summary ?? 'Qualified cohort · suppressed contacts excluded'], ['Subject', activity.previewTitle ?? activity.title]].map(([label, value]) => <div key={label} className="grid grid-cols-[4rem_1fr] border-b border-outline-variant/15 px-4 py-3 text-xs"><span className="text-on-surface-variant">{label}</span><strong className="min-w-0 break-words text-on-background">{value}</strong></div>)}
...
function EventCard({ activity, selected, onSelect }: { readonly activity: GrowthCalendarActivity; readonly selected: boolean; readonly onSelect: () => void }) {
  return (
    <button type="button" onClick={onSelect} className={`w-full rounded-xl border p-2 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary ${STATE_META[activity.displayState].card} ${selected ? 'ring-2 ring-on-background ring-offset-1' : ''}`}>
      <div className="flex items-start gap-2"><ChannelMark channel={activity.channel} small /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-1"><StateBadge state={activity.displayState} compact /><span className="shrink-0 text-[9px] opacity-70">{formatTime(activity.startsAt)}</span></div><p className="mt-1 line-clamp-2 text-[11px] font-bold leading-[0.9rem]">{activity.title}</p>{activity.statusLabel ? <p className="mt-0.5 text-[9px] font-semibold uppercase tracking-wide opacity-80">{activity.statusLabel}{typeof activity.recipientCount === 'number' ? ` · ${String(activity.recipientCount)} recipients` : ''}</p> : null}<div className="mt-1 text-[9px] opacity-70"><Agent owner={activity.owner} compact /></div></div></div>
    </button>
  );
}
