...
import { AI_CRAWLER_EXPLICIT_POLICY_DRIFT_PREFIX } from '../../../lib/shared/repair-signals';
...
export const aiCrawlerAccessCheck: AuditCheck = {
  ...
  run(ctx: CheckContext): CheckResult {
    ...
    const gateVerdicts = gate.map((agent) => ({
      agent,
      verdict: evaluateRobotsForToken(ctx.robotsTxtContent, agent.token, '/'),
    }));
    const blockedGate = gateVerdicts.filter(({ verdict }) => !verdict.allowed).map(({ agent }) => agent);
    ...
    const fallbackAgents = gateVerdicts
      .filter(({ verdict }) => verdict.matchedGroup !== 'specific')
      .map(({ agent }) => agent.token);
    if (fallbackAgents.length > 0) {
      const names = fallbackAgents.join(', ');
      return {
        id: 'ai-crawler-access',
        passed: true,
        status: 'WARNING',
        confidence: 'high',
        finding: `${AI_CRAWLER_EXPLICIT_POLICY_DRIFT_PREFIX} ${names}.`,
        fix: `Add explicit Allow: / groups for ${names} so a future wildcard policy change cannot accidentally remove search visibility.`,
      };
    }
