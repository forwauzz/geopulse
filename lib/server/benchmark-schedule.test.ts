...
describe('benchmark schedule helpers', () => {
  ...
  it.each(['benchmark_perplexity_quota_depleted', 'benchmark_perplexity_http_401'])('keeps a persisted %s terminal on the next hourly sweep', async (code) => {
    const runBenchmarkGroup = vi.fn().mockResolvedValue({
      runGroupId: 'run-auth-failed',
      queryRunCount: 10,
      completedQueryCount: 0,
    ...
    const repo = {
      ...
      getRunGroupByScheduleKey: vi.fn().mockResolvedValueOnce({ id: 'run-auth-failed', status: 'failed', metadata: { terminal_provider_failure_code: code } }).mockResolvedValue(null),
    };
    ...
    expect(runBenchmarkGroup).not.toHaveBeenCalled();
  });
