...
vi.mock("./distribution-social-oauth", async () => {
  const actual = await vi.importActual<
    typeof import("./distribution-social-oauth")
...
describe("dispatchDistributionJobs", () => {
    ...
    const repo = {
      ...
      updateJob: vi
        ...
        .mockResolvedValueOnce({
          ...
          provider_post_id: null,
          last_error:
            "No distribution account token is configured for provider x.",
          created_by_user_id: "user-1",
          completed_at: "2026-04-02T01:00:00.000Z",
          created_at: "2026-04-02T00:00:00.000Z",
          updated_at: "2026-04-02T00:00:00.000Z",
  ...
  it("publishes x single-image assets through the media publish path", async () => {
    ...
    const resolveAdapter = vi.fn(() => {
      throw new Error(
        "media publishers must not resolve content destination adapters",
      );
    });
    const summary = await dispatchDistributionJobById(
      supabase,
